...
const BUILD = 'V18 Rotating Partners Mode';
...
/** All k-combinations of `arr` of size `k` */
...
/** Canonical pair/group key – sorted player names joined */
function groupKey(names) {
  return [...names].sort().join('|');
}

/** All within-team pair keys for a group of player names */
function withinGroupPairKeys(names) {
  return combinations(names, 2).map(([a, b]) => groupKey([a, b]));
}
...
/** Round-robin schedule for a list of teams */
...
/** Validate before generating teams */
function validateTeamAsk(playerCount, teamCount, playersPerTeam) {
  if (!teamCount || teamCount < 2) return 'You need at least 2 teams.';
  if (playerCount < playersPerTeam * 2) return `Need at least ${playersPerTeam * 2} players for ${playersPerTeam}-player team mode.`;
  if (teamCount > playerCount) return 'Teams cannot exceed player count.';
  if (Math.floor(playerCount / teamCount) < playersPerTeam) {
    return `${playerCount} players cannot make ${teamCount} teams of ${playersPerTeam}. Try fewer teams.`;
  }
  return '';
}

/** For doubles (2-player) games: pick 3 matchup pairs from a team's players */
function randomizedPairOrder(players) {
  if (players.length < 2) return [];
  if (players.length === 2) return [[players[0], players[1]], [players[0], players[1]], [players[0], players[1]]];
  ...
  while (selected.length < 3) selected.push(allPairs[selected.length % allPairs.length]);
  return selected;
}

/** For singles (1-player) games: each slot is just the individual player wrapped in an array */
function singlesOrder(players) {
  return [[players[0]], [players[0]], [players[0]]];
}

/** Build team defs by shuffling players into `count` buckets */
function buildRandomTeamDefs(players, count) {
  ...
  shuffled.forEach((player, index) => defs[index % count].players.push(player));
  return defs;
}

/** Check if any team in candidateTeams repeats a within-team pair from usedSet */
function hasRepeatedPairs(candidateTeams, usedSet) {
  return candidateTeams.some((team) =>
    withinGroupPairKeys(team.players.map((p) => p.name)).some((k) => usedSet.has(k))
  );
}
...
export default function App() {
  ...
  const [pairHistory, setPairHistory] = useState([]); // rows from pair_history table
  ...
  // Rotating partners state
  const [exhaustionInfo, setExhaustionInfo] = useState(null); // { used, total, pct }

  const league = leagues.find((l) => l.id === leagueId);
  const week = weeks.find((w) => w.id === weekId);
  const playersPerTeam = league?.players_per_team ?? 2;
  ...
  useEffect(() => {
    if (weekId) { loadWeekData(); loadWeekCost(); }
    else clearWeekData();
  }, [weekId]);

  // Sync draft with incoming scores
  useEffect(() => {
    setDraft((current) => {
      const next = { ...current };
      scores.forEach((s) => {
  ...
  useEffect(() => {
    const channel = supabase.channel('ff-realtime-v18')
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'pair_history' }, () => loadPairHistory())
      .subscribe();
  ...
  function clearWeekData() {
    setPlayers([]); setTeams([]); setTeamPlayers([]);
    setMatches([]); setGames([]); setScores([]);
    setDraft({}); setWeekCost(null);
  }
  ...
  async function loadLeagues(selectFirst = true) {
    ...
    if (!data?.length) {
      const { data: newLeague } = await supabase
        .from('leagues').insert({ name: "Men's League", players_per_team: 2 }).select().single();
      const { data: newWeek } = await supabase
        .from('weeks').insert({ league_id: newLeague.id, name: 'Week 1' }).select().single();
  ...
  async function loadWeeks(selectFirst = true) {
    ...
    const { data, error: err } = await supabase
      .from('weeks').select('*').eq('league_id', leagueId).order('created_at', { ascending: false });
    if (err) return fail(err.message);

    const list = data || [];
    setWeeks(list);
    if (!list.length) { setWeekId(''); clearWeekData(); return; }
    if (selectFirst || !weekId || !list.find((w) => w.id === weekId)) setWeekId(list[0].id);
  }

  async function loadWeekData() {
    if (!weekId) { clearWeekData(); return; }
  ...
  async function loadPairHistory() {
    if (!leagueId) { setPairHistory([]); return; }
  ...
  function scoreFor(gameId) { return scores.find((s) => s.game_id === gameId) || {}; }
  ...
  function TeamLabel({ teamId }) {
    ...
    return (
      <div>
        <div className="pill" style={{ background: `${t.color}33`, color: t.color }}>{t.emoji} {t.name}</div>
  ...
  async function addPlayers() {
    ...
    await act(async () => {
      const { error: err } = await supabase
        .from('players').upsert(arr.map((name) => ({ week_id: weekId, name })), { onConflict: 'week_id,name' });
      if (err) throw err;
      setNames('');
    });
  }
  ...
  async function newLeague() {
    ...
    if (!name) return;
    const pptRaw = prompt('Players per team? (1 = singles, 2 = doubles)', '2');
    const ppt = Math.max(1, Math.min(6, Number(pptRaw) || 2));
    await act(async () => {
      const { data: newLeague, error: lErr } = await supabase
        .from('leagues').insert({ name, players_per_team: ppt }).select().single();
      if (lErr) throw lErr;
  ...
  async function newWeek() {
    ...
    if (!name) return;
    await act(async () => {
      const { data: newWeekRow, error: err } = await supabase
        .from('weeks').insert({ league_id: leagueId, name }).select().single();
      if (err) throw err;
  ...
  async function deleteWeek() {
    if (!weekId) return fail('No week is selected.');
    if (weeks.length <= 1) return fail('Cannot delete the only week. Create another first.');
    const nextWeek = weeks.find((w) => w.id !== weekId);
    if (!confirm(`Delete ${week?.name || 'this week'}?`)) return;
    await act(async () => {
  ...
  async function deleteLeague() {
    if (!leagueId) return fail('No league is selected.');
    if (leagues.length <= 1) return fail('Cannot delete the only league. Create another first.');
    const nextLeague = leagues.find((l) => l.id !== leagueId);
    if (!confirm('Delete entire league? All weeks in this league will be removed.')) return;
    await act(async () => {
  ...
  async function updateLeaguePlayersPerTeam(ppt) {
    if (!leagueId) return;
    const { error: err } = await supabase.from('leagues').update({ players_per_team: ppt }).eq('id', leagueId);
    if (err) return fail(err.message);
    setLeagues((ls) => ls.map((l) => l.id === leagueId ? { ...l, players_per_team: ppt } : l));
  }

  // ── Pair history management ───────────────────────────────────────────────

  /** The set of all used pair keys for this league */
  const usedPairKeys = useMemo(() => {
    const s = new Set();
    pairHistory.forEach((row) => s.add(groupKey([row.player_a, row.player_b])));
    return s;
  }, [pairHistory]);

  /** Exhaustion stats for the current player roster */
  const exhaustionStats = useMemo(() => {
    if (!players.length || playersPerTeam < 2) return null;
    const names = players.map((p) => p.name);
    const allPossible = combinations(names, playersPerTeam);
    const total = allPossible.length;
    // For 2-player mode count used pairs; for larger teams count used pair combos within team
    const usedCount = allPossible.filter((group) =>
      withinGroupPairKeys(group).every((k) => usedPairKeys.has(k))
    ).length;
    const pct = total > 0 ? Math.round((usedCount / total) * 100) : 0;
    return { used: usedCount, total, pct };
  }, [players, playersPerTeam, usedPairKeys]);

  async function recordPairsForTeams(teamDefs) {
    if (playersPerTeam < 2) return; // singles mode – no pairs to record
    const rows = [];
    teamDefs.forEach((team) => {
      const teamNames = team.players.map((p) => p.name);
      combinations(teamNames, 2).forEach(([a, b]) => {
    ...
    // upsert – silently skip duplicates
    await supabase.from('pair_history').upsert(rows, { onConflict: 'league_id,player_a,player_b', ignoreDuplicates: true });
    await loadPairHistory();
  }

  async function clearPairHistory() {
    if (!leagueId) return fail('No league selected.');
    if (!confirm('Clear ALL pair history for this league? This will allow all partner combinations again.')) return;
    await supabase.from('pair_history').delete().eq('league_id', leagueId);
    await loadPairHistory();
  }
  ...
  async function buildTeamsAndSchedule(teamDefs) {
    const minSize = playersPerTeam === 1 ? 1 : 2;
    if (teamDefs.some((t) => t.players.length < minSize)) {
      throw new Error(`Every team must have at least ${minSize} player(s).`);
    }
    ...
    const { data: insertedTeams, error: tErr } = await supabase
      .from('teams')
      .insert(teamDefs.map((d) => ({ week_id: weekId, name: d.name, emoji: d.emoji, color: d.color })))
      .select();
    if (tErr) throw tErr;

    const tpRows = [];
    insertedTeams.forEach((dbTeam, i) => {
      teamDefs[i].players.forEach((p) => tpRows.push({ team_id: dbTeam.id, player_id: p.id }));
    });
    const { error: tpErr } = await supabase.from('team_players').insert(tpRows);
    if (tpErr) throw tpErr;

    // Round-robin schedule
    const rounds = generateRoundRobinRounds(insertedTeams);
    const matchRows = [];
    rounds.forEach((round, slot) => {
      round.forEach(([teamA, teamB], court) => {
        matchRows.push({
          week_id: weekId,
          slot: slot + 1,
    ...
    insertedMatches.forEach((match) => {
      const t1Players = teamMap[match.team1_id];
      const t2Players = teamMap[match.team2_id];

      let t1Slots, t2Slots;
      if (playersPerTeam === 1) {
        // Singles: player represents themselves
        t1Slots = singlesOrder(t1Players);
        t2Slots = singlesOrder(t2Players);
      } else {
        // Doubles or larger: rotate pairs within a team
        t1Slots = randomizedPairOrder(t1Players);
        t2Slots = randomizedPairOrder(t2Players);
      }

      [0, 1, 2].forEach((n) => {
        const t1 = t1Slots[n % t1Slots.length];
        const t2 = t2Slots[n % t2Slots.length];
        gameRows.push({
          match_id: match.id,
          game_number: n + 1,
          t1_player1_id: t1[0].id,
          t1_player2_id: (t1[1] || t1[0]).id,
          t2_player1_id: t2[0].id,
          t2_player2_id: (t2[1] || t2[0]).id,
        });
      });
    });
    ...
    // Record the pairs used so they never repeat
    await recordPairsForTeams(teamDefs);
  }

  // ── Auto team generation with uniqueness enforcement ──────────────────────

  function findUniqueTeamDefs(count, maxAttempts = 20000) {
    // Level 1: avoid ALL previously used pairs
    for (let i = 0; i < maxAttempts; i++) {
      const candidate = buildRandomTeamDefs(players, count);
      if (!hasRepeatedPairs(candidate, usedPairKeys)) {
        return { defs: candidate, relaxed: false };
      }
    }
    return { defs: null, relaxed: true };
  }

  async function randomTeams() {
    if (!weekId) return fail('No week is selected. Create or select a week first.');

    const suggestedTeamCount = Math.max(2, Math.ceil(players.length / playersPerTeam));
    const response = prompt(
      `Number of teams? (${playersPerTeam}-player teams, ${players.length} players)`,
      suggestedTeamCount
    );
    if (response === null) return;

    const count = Number(response || suggestedTeamCount);
    const validation = validateTeamAsk(players.length, count, playersPerTeam);
    if (validation) return fail(validation);

    await act(async () => {
      const result = findUniqueTeamDefs(count);

      if (!result.defs) {
        const proceed = confirm(
          'All unique partner combinations have been used for this roster!\n\n' +
          'Partners will repeat. Continue anyway?'
        );
        if (!proceed) throw new Error('Team generation cancelled — all unique partnerships exhausted.');
        // Fallback: just generate random without uniqueness
        result.defs = buildRandomTeamDefs(players, count);
      }
  ...
  async function saveManualTeams() {
    ...
    const assignedIds = Object.keys(manual.assignments).filter(
      (pid) => manual.assignments[pid] !== '' && manual.assignments[pid] != null
    );
    if (assignedIds.length !== players.length) {
      return fail(`Assign all players. ${players.length - assignedIds.length} unassigned.`);
    }
    ...
    const smallTeam = defs.find((t) => t.players.length < (playersPerTeam === 1 ? 1 : 2));
    if (smallTeam) return fail(`${smallTeam.name} has too few players.`);

    await act(() => buildTeamsAndSchedule(defs));
  }
  ...
  async function saveGameScore(gameId) {
    ...
    if ((s1 === '' && s2 !== '') || (s1 !== '' && s2 === '')) return fail('Enter both scores, or clear both.');

    await act(async () => {
      const { error: err } = await supabase.rpc('update_score_with_history', {
        p_game_id: gameId, p_score1: s1 === '' ? null : Number(s1), p_score2: s2 === '' ? null : Number(s2),
      });
      if (err) throw err;
      setDraft((cur) => { const next = { ...cur }; delete next[gameId]; return next; });
    });
  }

  async function resetGameScore(gameId) {
    await act(async () => {
      const { error: err } = await supabase.rpc('update_score_with_history', { p_game_id: gameId, p_score1: null, p_score2: null });
      if (err) throw err;
  ...
  async function resetMatchScores(matchId) {
    ...
    await act(async () => {
      await Promise.all(matchGames.map((g) =>
        supabase.rpc('update_score_with_history', { p_game_id: g.id, p_score1: null, p_score2: null })
      ));
      setDraft((d) => {
        const next = { ...d };
  ...
  async function loadOverallLeaderboard() {
    if (!leagueId) { setOverallRows([]); setOverallCalculatedAt(null); return; }
    const { data } = await supabase.from('overall_leaderboards').select('*').eq('league_id', leagueId).maybeSingle();
  ...
  async function calculateOverallLeaderboard() {
    if (!leagueId) return fail('No league is selected.');
    await act(async () => {
      const { data: leagueWeeks, error: wErr } = await supabase.from('weeks').select('id,name').eq('league_id', leagueId);
      if (wErr) throw wErr;
      const weekIds = (leagueWeeks || []).map((w) => w.id);
      if (!weekIds.length) return fail('This league has no weeks.');
      ...
      if (!matchIds.length) {
        await supabase.from('overall_leaderboards').upsert({ league_id: leagueId, data: [], calculated_at: new Date().toISOString() });
        setOverallRows([]);
        return;
      }

      const { data: allGames, error: gErr } = await supabase.from('match_games').select('*').in('match_id', matchIds);
      if (gErr) throw gErr;
      ...
      (allGames || []).forEach((game) => {
        ...
        [game.t1_player1_id, game.t1_player2_id].forEach((id) => {
          ...
          // dedupe for singles (t1_player1_id === t1_player2_id)
          if (id === game.t1_player1_id || id !== game.t1_player2_id) {
            const row = ensure(p.name);
            row.played++; row.pointsFor += s1; row.pointsAgainst += s2;
            if (s1 > s2) row.wins++; else if (s2 > s1) row.losses++;
          }
        });
        // dedupe: if both player slots are same id (singles), skip the second
        const t1Unique = game.t1_player1_id === game.t1_player2_id
          ? [game.t1_player1_id] : [game.t1_player1_id, game.t1_player2_id];
        const t2Unique = game.t2_player1_id === game.t2_player2_id
          ? [game.t2_player1_id] : [game.t2_player1_id, game.t2_player2_id];
        ...
        t2Unique.forEach((id) => {
          const p = playerById[id]; if (!p) return;
          const row = ensure(p.name);
          row.played++; row.pointsFor += s2; row.pointsAgainst += s1;
          if (s2 > s1) row.wins++; else if (s1 > s2) row.losses++;
        });
      });
      ...
      const { error: upsertErr } = await supabase.from('overall_leaderboards').upsert({ league_id: leagueId, data: ranked, calculated_at: calculatedAt });
  ...
  async function clearOverallLeaderboard() {
    ...
    await act(async () => {
      ...
      const calculatedAt = new Date().toISOString();
      const { error: err } = await supabase.from('overall_leaderboards').upsert({ league_id: leagueId, data: [], calculated_at: calculatedAt });
      if (err) throw err;
      setOverallRows([]);
      setOverallCalculatedAt(calculatedAt);
    });
  ...
  async function undoOverallLeaderboard() {
    ...
    await act(async () => {
      const { data: last, error: hErr } = await supabase
        .from('overall_leaderboard_history').select('*').eq('league_id', leagueId)
        .order('created_at', { ascending: false }).limit(1).maybeSingle();
      if (hErr) throw hErr;
      if (!last) return fail('No previous leaderboard to restore.');
      const calculatedAt = new Date().toISOString();
      const { error: upsertErr } = await supabase.from('overall_leaderboards').upsert({ league_id: leagueId, data: last.data || [], calculated_at: calculatedAt });
      if (upsertErr) throw upsertErr;
  ...
  async function loadWeekCost() {
    if (!weekId) { setWeekCost(null); return; }
    const { data } = await supabase.from('week_costs').select('*').eq('week_id', weekId).maybeSingle();
    setWeekCost(data || { week_id: weekId, cost_per_birdie: 0, birdie_count: 0, court_booking_cost: 0, player_count_override: null });
  }

  function updateCostDraft(field, value) {
    const clean = value === '' ? '' : String(value).replace(/[^0-9.]/g, '');
    setWeekCost((current) => ({
      ...(current || { week_id: weekId, cost_per_birdie: 0, birdie_count: 0, court_booking_cost: 0, player_count_override: null }),
      [field]: clean,
    }));
  }
  ...
  async function saveWeekCost() {
    ...
    const payload = {
      ...
      player_count_override: weekCost?.player_count_override === '' || weekCost?.player_count_override == null
        ? null : Number(weekCost.player_count_override),
  ...
  async function clearWeekCost() {
    ...
    await act(async () => {
      ...
      if (err) throw err;
      setWeekCost({ week_id: weekId, cost_per_birdie: 0, birdie_count: 0, court_booking_cost: 0, player_count_override: null });
    });
  }
  ...
  const playerStandings = useMemo(() => {
    ...
    games.forEach((game) => {
      ...
      const a = Number(score.score1), b = Number(score.score2);
      const t1Ids = game.t1_player1_id === game.t1_player2_id
        ? [game.t1_player1_id] : [game.t1_player1_id, game.t1_player2_id];
      const t2Ids = game.t2_player1_id === game.t2_player2_id
        ? [game.t2_player1_id] : [game.t2_player1_id, game.t2_player2_id];
      t1Ids.forEach((id) => {
        if (!stats[id]) return;
        stats[id].played++; stats[id].pointsFor += a; stats[id].pointsAgainst += b;
        if (a > b) stats[id].wins++; else if (b > a) stats[id].losses++;
      });
      t2Ids.forEach((id) => {
        if (!stats[id]) return;
        stats[id].played++; stats[id].pointsFor += b; stats[id].pointsAgainst += a;
        if (b > a) stats[id].wins++; else if (a > b) stats[id].losses++;
      });
    });
  ...
  const teamStandings = useMemo(() => {
    ...
    matches.forEach((match) => {
      ...
      const A = stats[match.team1_id], B = stats[match.team2_id];
      if (!A || !B) return;
  ...
  const nav = [
    ...
    ['partners', 'Partner History', RefreshCw],
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <div className="card">
          ...
          <p className="buildMarker">{BUILD}</p>
          <p className="muted">
            Mode: <b style={{ color: '#facc15' }}>{playersPerTeam === 1 ? 'Singles' : `${playersPerTeam}-Player Teams`}</b>
          </p>
          {exhaustionStats && playersPerTeam >= 2 && (
            <p className="muted">
              Partner combos used: <b style={{ color: exhaustionStats.pct >= 90 ? '#fb7185' : '#4ade80' }}>
                {exhaustionStats.used}/{exhaustionStats.total} ({exhaustionStats.pct}%)
              </b>
            </p>
          )}
      ...
      <main className="main">
        {error && (
          <div className="error">
            ...
            <button className="btn secondary" style={{ marginLeft: 12, padding: '4px 10px' }} onClick={() => setError('')}>✕</button>
          </div>
        )}
          ...
          <div className="grid stats">
            ...
            {exhaustionStats && playersPerTeam >= 2 && (
              <div className="card stat">
                <RefreshCw className="statIcon" style={{ color: exhaustionStats.pct >= 90 ? '#fb7185' : '#4ade80' }} />
                <div>
                  <span className="muted">Partner Combos Used</span>
                  <b style={{ color: exhaustionStats.pct >= 90 ? '#fb7185' : '#4ade80' }}>
                    {exhaustionStats.used}/{exhaustionStats.total}
                  </b>
                </div>
              </div>
            )}
              ...
              <tbody>
                {players.map((p) => (
                  <tr key={p.id}>
                    <td>{p.name}</td>
                    <td style={{ width: 120 }}>
                      <button className="btn danger" onClick={() => removePlayer(p)}><Trash2 size={16} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {tab === 'teams' && (
          <div className="card">
            ...
            <div className="card" style={{ marginBottom: 16 }}>
              ...
              <p className="muted">
                Changing this affects how teams are built and whether partner uniqueness is enforced.
                Current setting for <b>{league?.name}</b>: <b style={{ color: '#facc15' }}>{playersPerTeam}</b>
              </p>
              <div className="row">
                {[1, 2, 3, 4].map((n) => (
                  <button
                    key={n}
                    className={playersPerTeam === n ? 'btn' : 'btn secondary'}
                    onClick={() => updateLeaguePlayersPerTeam(n)}
                  >
                    {n === 1 ? '1 – Singles' : `${n} – ${n === 2 ? 'Doubles' : `${n}-player`}`}
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Exhaustion warning */}
            {exhaustionStats && playersPerTeam >= 2 && exhaustionStats.pct >= 80 && (
              <div className="exhaustionWarning">
                <Info size={16} />
                <span>
                  {exhaustionStats.pct >= 100
                    ? '⚠️ All unique partner combinations have been used! Partners will repeat on next generate.'
                    : `⚠️ ${exhaustionStats.pct}% of unique partner combos used (${exhaustionStats.used}/${exhaustionStats.total}).`
                  }
                  <button className="btn secondary" style={{ marginLeft: 12, padding: '4px 10px' }} onClick={() => setTab('partners')}>View History</button>
                </span>
              </div>
            )}
            ...
            {mode === 'auto' ? (
              <div className="card">
                ...
                <p className="muted">
                  Generates teams ensuring no two players have been partners before in this league.
                  {playersPerTeam === 1 ? ' Singles mode: no partner tracking needed.' : ''}
                </p>
              ...
              <div className="card">
                ...
                <div className="row">
                  <label>Number of Teams
                    <input type="text" inputMode="numeric" value={manual.teamCount}
                      onChange={(e) => setManual({ ...manual, teamCount: e.target.value })} />
                  </label>
                    ...
                    <div className="card" key={i}>
                      ...
                      {players.map((p) => (
                        <label className="checkrow" key={p.id}>
                          <input type="radio" name={`assign-${p.id}`}
                ...
                {games.filter((g) => g.match_id === match.id).sort((a, b) => a.game_number - b.game_number).map((game) => {
                  ...
                  return (
                    <div className="game" key={game.id}>
                      <div>{playerName(game.t1_player1_id)}{game.t1_player1_id !== game.t1_player2_id ? ` / ${playerName(game.t1_player2_id)}` : ''}</div>
                      <input className="score" type="text" inputMode="numeric" pattern="[0-9]*"
                        value={shown.score1 ?? ''} onChange={(e) => draftChange(game.id, 'score1', e.target.value)} />
                      <input className="score" type="text" inputMode="numeric" pattern="[0-9]*"
                        value={shown.score2 ?? ''} onChange={(e) => draftChange(game.id, 'score2', e.target.value)} />
                      <div>{playerName(game.t2_player1_id)}{game.t2_player1_id !== game.t2_player2_id ? ` / ${playerName(game.t2_player2_id)}` : ''}</div>
                      <div className="row">
        ...
        {tab === 'costs' && (
          <div className="card">
            ...
            <div className="fireline" />
            <p className="muted">Enter the birdie cost, number of birdies used, court booking cost, and player count.</p>
            <div className="grid stats">
            ...
            <div className="card">
              <div className="row">
                <label>Cost per birdie
                  <input type="text" inputMode="decimal" value={weekCost?.cost_per_birdie ?? ''}
                    onChange={(e) => updateCostDraft('cost_per_birdie', e.target.value)} placeholder="e.g. 3.50" />
                </label>
                <label>Number of birdies
                  <input type="text" inputMode="decimal" value={weekCost?.birdie_count ?? ''}
                    onChange={(e) => updateCostDraft('birdie_count', e.target.value)} placeholder="e.g. 12" />
                </label>
              </div>
              <div className="row" style={{ marginTop: 12 }}>
                <label>Court booking cost
                  <input type="text" inputMode="decimal" value={weekCost?.court_booking_cost ?? ''}
                    onChange={(e) => updateCostDraft('court_booking_cost', e.target.value)} placeholder="e.g. 160" />
                </label>
                <label>Number of players
                  <input type="text" inputMode="numeric" value={weekCost?.player_count_override ?? ''}
                    onChange={(e) => updateCostDraft('player_count_override', e.target.value)} placeholder={`Default: ${players.length}`} />
                </label>
              </div>
        ...
        {tab === 'partners' && (
          <div className="card">
            ...
            <div className="fireline" />
            <p className="muted">
              All unique partner pairs that have been used in <b>{league?.name}</b>.
              Auto-generation avoids repeating any of these.
            </p>
            ...
            {exhaustionStats && playersPerTeam >= 2 && (
              <div className="grid stats" style={{ marginBottom: 16 }}>
                <div className="card stat">
                  <RefreshCw className="statIcon" style={{ color: exhaustionStats.pct >= 90 ? '#fb7185' : '#4ade80' }} />
                  <div>
                    <span className="muted">Combos Used</span>
                    <b style={{ color: exhaustionStats.pct >= 90 ? '#fb7185' : '#4ade80' }}>
                      {exhaustionStats.used} / {exhaustionStats.total}
                    </b>
                  </div>
                ...
                <div className="card stat">
                  <div>
                    <span className="muted">Uniqueness Remaining</span>
                    <b style={{ color: exhaustionStats.pct >= 90 ? '#fb7185' : '#4ade80' }}>
                      {100 - exhaustionStats.pct}%
                    </b>
                  </div>
                </div>
                <div className="card stat">
                  <div>
                    <span className="muted">Total Pairs Logged</span>
                    <b>{pairHistory.length}</b>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="row" style={{ marginBottom: 16 }}>
              <button className="btn danger" onClick={clearPairHistory}>Clear All Pair History</button>
            </div>

            {pairHistory.length === 0 ? (
              <p className="muted">No pair history yet. Generate teams and it will be tracked automatically.</p>
            ) : (
              <table>
                <thead>
                  <tr><th>#</th><th>Player A</th><th>Player B</th><th>Week</th></tr>
                </thead>
                <tbody>
                  {pairHistory
                    .slice()
                    ...
                    .map((row, i) => (
                      <tr key={row.id}>
                        ...
                        <td><b>{row.player_a}</b></td>
                        <td><b>{row.player_b}</b></td>
                        <td className="muted">{weeks.find((w) => w.id === row.week_id)?.name || '—'}</td>
                      </tr>
                    ))}
                </tbody>
              </table>
          ...
          <div className="card">
            ...
            <div className="row" style={{ marginTop: 12 }}>
              ...
              <button className="btn secondary" onClick={() => {
                ...
                a.href = URL.createObjectURL(new Blob([JSON.stringify({ leagues, weeks, players, teams, teamPlayers, matches, games, scores, pairHistory }, null, 2)], { type: 'application/json' }));
            ...
            <div className="card" style={{ marginTop: 16 }}>
              ...
              <p className="muted">Players per team for <b>{league?.name}</b>:</p>
              <div className="row">
                {[1, 2, 3, 4].map((n) => (
                  <button key={n} className={playersPerTeam === n ? 'btn' : 'btn secondary'} onClick={() => updateLeaguePlayersPerTeam(n)}>
                    {n === 1 ? '1 – Singles' : `${n} – ${n === 2 ? 'Doubles' : `${n}-player`}`}
                  </button>
                ))}
              </div>
          ...
          <tbody>
            {rows.map((s, i) => (
              <tr key={s.team.id}>
                <td>{i + 1}</td>
                <td>
                  <div className="pill" style={{ background: `${s.team.color}33`, color: s.team.color }}>{s.team.emoji} {s.team.name}</div>
